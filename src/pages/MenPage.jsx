...
/* ─── Constants ───────────────────────────────────────────────── */
const CATEGORIES = [
  'All','Kits','TShirts','Shirt','Office Wear','Party Wear','Formal Wear','Casual Wear','Pant','Cargo Pant','Shorts','Jeans','Denim','Joggers','Tracksuits','Jersey','Hoddie Top','Sweatshirts','Jackets','Blazer','Overcoat','Waistcoat','Budget Tees','Gym Wear','Activewear','Beach Wear','Lungi'
];
...
const RATING_BADGES = [
  { min:4.5, label:'Top Rated', icon:<Star size={9} fill="currentColor"/> , color:'#f5c842', bg:'#fffbea' },
  { min:0,   label:'New',       icon:<Zap size={9}/>,                         color:'#5b8ed6', bg:'#eef4ff' },
];
...
export default function MenPage(){
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await supabase
        .from('products').select('*').eq('gender_category','Men');
  ...
  const ProductCard = ({ p }) => {
    ...
    // Image carousel state — local to each card
      ...
      <>
        ...
        >
          ...
          <div style={{position:'relative', height: gridCols===1?'280px':'330px', overflow:'hidden', background:'#f8f8f8'}}>
            ...
            {/* Manual prev/next arrows on hover */}
        ...
        {/* ── Hover Popup ── rendered outside card via portal-style fixed overlay */}
        {showPopup && (
  ...
  return (
    <div style={{backgroundColor:'#f7f7f7',minHeight:'100vh',fontFamily:'"Outfit",sans-serif'}}>
      ...
      {/* ── Hero ── full bleed, no margin, no radius */}
      <div style={{
        ...
        {/* gradient: darker bottom so text pops */}
        <div style={{
          position:'absolute', inset:0,
          ...
          {sidebarOpen && (
            <div>
              ...
              <SidebarSection id="price" label="PRICE">
                ...
                {/* dual range - simple max-price for now */}
                <input
