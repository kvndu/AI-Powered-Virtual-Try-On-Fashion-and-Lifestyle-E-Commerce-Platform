...
export default function AccessoriesPage() {
  ...
  const [expandedPaths,     setExpandedPaths]     = useState({ "Women's Accessories": true });
  ...
  const filteredProducts = useMemo(() => {
    let r = products.filter(p => p.gender_category?.trim().toLowerCase() === 'women');
    ...
    if (selectedCategory.level === 1) {
      const targetGender = selectedCategory.gender || 'Women';
      ...
      r = r.filter(p => p.subcategory?.trim().toLowerCase() === targetSub.toLowerCase());
    }
