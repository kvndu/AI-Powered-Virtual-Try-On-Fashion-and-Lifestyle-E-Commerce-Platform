...
/* ─── Constants ───────────────────────────────────────────────── */
const CATEGORIES = [
  'All','Kits','Pant','Dresses','Office Wear','Saree','Frocks',
  'Blouse','Hoddie Top','Skirt','Jersey','Cargo Pant','Shorts',
  'Denim','Jackets','Romper','Jumpsuits','Shirt','Tops','Dungaree',
  'Party Wear','TShirts','Crop Top','Skinny','Budget Tees',
  'Overcoat','Belts','Pinafore','Blazer','Lungi',
];
...
const STYLE_COLLECTIONS = [
  { label: 'New Arrivals', iconKey: 'Sparkles', desc: 'Fresh drops this week' },
  { label: 'Best Sellers', iconKey: 'Flame', desc: 'Customer favorites' },
  { label: 'Trending Now', iconKey: 'TrendingUp', desc: "What's hot right now" },
  { label: 'Festival Edit', iconKey: 'Star', desc: 'Festive season picks' },
];

const ICON_MAP = { Sparkles, Flame, TrendingUp, Star };
...
export default function WomenPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await supabase
        .from('products').select('*').eq('gender_category','Women');
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (!selectedCats.includes('All'))
      r = r.filter(p => selectedCats.includes(p.category));
    r = r.filter(p => p.price_lkr >= priceRange[0] && p.price_lkr <= priceRange[1]);
    
    // Apply Mood Collections filter
    if (activeCollection === 'New Arrivals') {
      r = r.filter(p => {
        const isNew = !p.created_at || (Date.now() - new Date(p.created_at)) < 1000*60*60*24*30;
        return isNew;
      });
    } else if (activeCollection === 'Best Sellers' || activeCollection === 'Trending Now') {
      r = r.filter(p => p.original_price && p.price_lkr < p.original_price); // Sale or discount
    } else if (activeCollection === 'Festival Edit') {
      r = r.filter(p => p.category === 'Saree' || p.category === 'Party Wear');
    }
  ...
  const clearAll = () => {
    setSelectedCats(['All']); setPriceRange([0,10000]);
  ...
  const ProductCard = ({ p, onProductClick }) => {
    ...
    const images = (p.images_array && p.images_array.length > 0)
      ? p.images_array
      : [`https://via.placeholder.com/400x400/111111/C9A96E?text=${encodeURIComponent(p.name||'Product')}`];
  ...
  return (
    <div style={{backgroundColor:'#080808',minHeight:'100vh',fontFamily:'"Outfit",sans-serif',color:'#fff',overflowX:'hidden'}}>
      ...
      <section style={{
        ...
        <div style={{position:'relative',zIndex:2,textAlign:'center',maxWidth:'900px'}}>
          ...
          <h1 style={{
            ...
            WO<span style={{color:'#C9A96E'}}>MEN</span>
          </h1>
          ...
          <p style={{
            ...
            Timeless elegance meets modern craft. Discover pieces made for every moment of your story.
          </p>

          <div style={{display:'flex',gap:'28px',justifyContent:'center',flexWrap:'wrap'}}>
            {STYLE_COLLECTIONS.map(item=>(
              <span
                key={item.label}
                onClick={() => {
                ...
                style={{
                  fontSize:'10px',color: activeCollection === item.label ? '#C9A96E' : 'rgba(255,255,255,0.75)',
                  letterSpacing:'3px',
                  cursor:'pointer',borderBottom: activeCollection === item.label ? '2px solid #C9A96E' : '1px solid rgba(201,169,110,0.4)',
      ...
      {/* MARQUEE */}
      <div style={{background:'#C9A96E',overflow:'hidden',padding:'14px 0',borderTop:'1px solid rgba(0,0,0,0.1)',borderBottom:'1px solid rgba(0,0,0,0.1)'}}>
        <div className="women-marquee-track">
      ...
      <div id="products-section" style={{
        ...
        <span style={{fontSize:'14px',color:'rgba(255,255,255,0.28)',letterSpacing:'2px',whiteSpace:'nowrap',fontFamily:'"Bebas Neue",sans-serif'}}>
          {loading ? '...' : filteredProducts.length + ' / ' + products.length + ' ITEMS'}
        </span>
      ...
      {activeFilterCount > 0 && (
        <div style={{padding:'10px 40px',display:'flex',gap:'8px',flexWrap:'wrap',alignItems:'center',background:'rgba(8,8,8,0.98)',borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
          ...
          {activeCollection && <FilterChip label={'Mood: ' + activeCollection} onRemove={()=>setActiveCollection(null)}/>}
          <button
            onClick={clearAll}
            style={{fontSize:'14px',color:'#C9A96E',background:'none',border:'none',cursor:'pointer',fontWeight:'900',letterSpacing:'2px',fontFamily:'"Bebas Neue",sans-serif'}}
          >
            CLEAR ALL
          </button>
        </div>
              ...
              <SidebarSection id="price" label="PRICE RANGE">
                ...
                <input
                  ...
                  style={{
                    ...
                    background:'linear-gradient(to right,#C9A96E ' + priceRange[1]/100 + '%,rgba(255,255,255,0.1) ' + priceRange[1]/100 + '%)',
                    outline:'none',border:'none',cursor:'pointer',
                  }}
                />
                <div style={{display:'flex',gap:'6px',marginTop:'16px',flexWrap:'wrap'}}>
                  {[[0,2000,'Under 2K'],[2000,5000,'2K - 5K'],[5000,10000,'5K+']].map(([mn,mx,lbl])=>(
                    <button
                      key={lbl}
      ...
      <style>{`
        ...
        .women-marquee-track {
          display: flex;
          width: max-content;
          animation: womenMarqueeLoop 28s linear infinite;
        }
        @keyframes womenShimmer {
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(' + cols + ',1fr)',gap:'20px'}}>
      {[...Array(cols*2)].map((_,i)=>(
        <div key={i} style={{overflow:'hidden',border:'1px solid rgba(255,255,255,0.06)'}}>
          <div style={{height:'340px',background:'linear-gradient(90deg,rgba(255,255,255,0.04) 25%,rgba(255,255,255,0.08) 50%,rgba(255,255,255,0.04) 75%)',backgroundSize:'200% 100%',animation:'womenShimmer 1.6s infinite'}}/>
...
function EmptyState({ onClear }) {
  return (
    <div style={{textAlign:'center',padding:'120px 0',display:'flex',flexDirection:'column',alignItems:'center',gap:'20px'}}>
      ...
      <div>
        <p style={{color:'rgba(255,255,255,0.2)',margin:'0 0 8px',fontFamily:'"Bebas Neue",sans-serif',fontSize:'22px',letterSpacing:'3px'}}>NO STYLES FOUND</p>
