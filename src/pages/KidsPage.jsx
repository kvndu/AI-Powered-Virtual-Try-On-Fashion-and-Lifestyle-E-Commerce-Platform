...
const GIRLS_CATEGORIES = [
  'Dresses', 'Tops', 'T-Shirts', 'Skirts', 'Leggings', 'Jeans', 'Shorts',
  'Sets & Co-Ords', 'Party Wear', 'Ethnic Wear', 'Sleepwear', 'Sportswear',
  'Jackets', 'Hoodies', 'Sweatshirts', 'Shoes', 'Accessories'
];

const BOYS_CATEGORIES = [
  'T-Shirts', 'Shirts', 'Jeans', 'Shorts', 'Pants', 'Sets & Co-Ords',
  'Party Wear', 'Ethnic Wear', 'Sleepwear', 'Sportswear', 'Jackets',
  'Hoodies', 'Sweatshirts', 'Shoes', 'Accessories'
];
...
const AGE_GROUP_SIZES = {
  'Infant (0-12M)': ['0-6M', '6-12M'],
  'Toddler (1-3Y)': ['1Y', '2Y', '3Y'],
  'Little Kid (4-7Y)': ['4Y', '5Y', '6Y', '7Y'],
  'Big Kid (8-12Y)': ['8Y', '10Y', '12Y'],
  'Teen (13-16Y)': ['14Y', '16Y']
};

const SIZES = [
  '0-6M', '6-12M', '1Y', '2Y', '3Y', '4Y', '5Y', '6Y', '7Y', '8Y', '10Y', '12Y', '14Y', '16Y'
];

const COLORS = [
  { name: 'Oatmeal', hex: '#eae3d2' },
  { name: 'Cream', hex: '#f8f0e0', border: '#ccc' },
  { name: 'Peach', hex: '#ffdab9' },
  { name: 'Dusty Pink', hex: '#dcaebb' },
...
const MOCK_KIDS_PRODUCTS = [
  {
    id: '69fc7617-902c-4f8c-87d8-ccbf97cc144f',
    name: 'PLAIN WHITE FORMAL TOP',
    price_lkr: 2595,
    original_price: 2595,
    category: 'Tops',
    gender: 'Girls',
    ...
    images_array: [
      'https://nliaslftsujgvuwwnltx.supabase.co/storage/v1/object/public/product-images/k.jpeg',
      'https://nliaslftsujgvuwwnltx.supabase.co/storage/v1/object/public/product-images/kl.jpeg',
      'https://nliaslftsujgvuwwnltx.supabase.co/storage/v1/object/public/product-images/km.jpeg'
    ],
    description: 'A plain white formal top imported directly from the women collection, serving as a placeholder for kids cards.'
  }
];
...
export default function KidsPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data, error } = await supabase
        .from('products')
        .select('*')
        .eq('gender_category', 'Kids');
      
      if (data && data.length > 0) {
        setSelectedProducts(data);
      } else {
        // Fallback to mock data for layout design preview
        setSelectedProducts(MOCK_KIDS_PRODUCTS);
      }
      setLoading(false);
    })();
  }, []);
  ...
  const maxProductPrice = useMemo(() => {
    ...
    return Math.max(...products.map(p => p.price_lkr), 15000);
  }, [products]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    // Category filtering (visual separation handles Girls - Dresses and Boys - Shirts)
    if (!selectedCats.includes('All')) {
      r = r.filter(p => {
        return selectedCats.some(selCat => {
          if (selCat === 'All') return true;
          const parts = selCat.split(' - ');
          if (parts.length === 2) {
            const [gender, categoryName] = parts;
            return (
              (p.gender?.toLowerCase() === gender.toLowerCase() || p.sub_gender?.toLowerCase() === gender.toLowerCase()) &&
              p.category?.toLowerCase() === categoryName.toLowerCase()
            );
          }
          return p.category?.toLowerCase() === selCat.toLowerCase();
        });
      });
    }
    ...
    if (!selectedAgeGroups.includes('All')) {
      r = r.filter(p => {
        const allowedSizes = selectedAgeGroups.flatMap(ag => AGE_GROUP_SIZES[ag] || []);
        const productSizes = p.sizes || (p.size ? [p.size] : []);
        if (productSizes.length > 0) {
          return productSizes.some(sz => allowedSizes.includes(sz));
        }
        if (p.age_group) {
          return selectedAgeGroups.includes(p.age_group);
        }
        return true;
      });
    }
    ...
    if (sortBy === 'popular') r.sort((a, b) => (b.popularity || 0) - (a.popularity || 0));

    return r;
  }, [products, selectedCats, selectedAgeGroups, priceRange, selectedSizes, selectedColors, search, sortBy]);
  ...
  const toggleAgeGroup = (ag) => {
    if (ag === 'All') { setSelectedAgeGroups(['All']); return; }
    setSelectedAgeGroups(prev => prev.includes(ag) ? ['All'] : [ag]);
  };

  const toggleSize = (s) => {
    setSelectedSizes(prev => prev.includes(s) ? [] : [s]);
  };
  ...
  const visibleGirlsCats = catSearch
    ? GIRLS_CATEGORIES.filter(c => c.toLowerCase().includes(catSearch.toLowerCase()))
    : GIRLS_CATEGORIES;

  const visibleBoysCats = catSearch
    ? BOYS_CATEGORIES.filter(c => c.toLowerCase().includes(catSearch.toLowerCase()))
    : BOYS_CATEGORIES;
          ...
          <div style={{ padding: '14px 16px 16px' }}>
            {p.category && (
              <span style={{ fontSize: '9px', color: '#bbb', letterSpacing: '2px', fontWeight: '600' }}>
                {(p.gender ? `${p.gender} · ` : '') + p.category.toUpperCase()}
              </span>
            )}
            <h3 style={{
        ...
        <div style={{ padding: '16px 18px 18px' }}>
          ...
          {p.sizes && (
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '14px' }}>
              {p.sizes.map(sz => (
                <span key={sz} style={{
                  fontSize: '9px', fontWeight: '600', padding: '3px 7px',
                  border: '1px solid #e8e8e8', borderRadius: '6px', color: '#888'
                }}>{sz}</span>
              ))}
            </div>
          )}
  ...
  return (
    <div style={{ backgroundColor: '#f7f7f7', minHeight: '100vh', fontFamily: '"Outfit",sans-serif' }}>
      ...
      {/* ── Hero ── full bleed, no margin, no radius, custom soft warm pastel background */}
      <div style={{
        position: 'relative', height: '380px', overflow: 'hidden',
        width: '100%',
        background: 'linear-gradient(135deg, #f5efeb 0%, #e8dbd2 50%, #dcd0c9 100%)'
      }}>
        {/* elegant dark gradient layer for visual contrast */}
        <div style={{
          position: 'absolute', inset: 0,
        ...
        <div style={{
          ...
          <span style={{ fontSize: '10px', letterSpacing: '6px', color: '#8c7e75', fontWeight: '700' }}>
            BLOOMAIR · KIDS COLLECTION
          </span>
          <h1 style={{
            color: '#2c221e', fontSize: '72px', letterSpacing: '6px',
            fontWeight: '300', margin: '0',
            fontFamily: '"Playfair Display",Georgia,serif', lineHeight: 1,
          }}>
            Kids Fashion
          </h1>
          <p style={{ color: '#685951', fontSize: '14px', letterSpacing: '1px', margin: '4px 0 10px 0', fontWeight: '500' }}>
            Fashion for Every Little Adventure
          </p>
          <div style={{ display: 'flex', gap: '28px', marginTop: '6px' }}>
            {['New Arrivals', 'Best Sellers', 'Trending'].map(tag => (
              <span key={tag} style={{
                fontSize: '11px', color: '#2c221e', letterSpacing: '2.5px',
                cursor: 'pointer', borderBottom: '1px solid rgba(44,34,30,0.4)',
                paddingBottom: '3px', transition: 'color 0.2s', fontWeight: '600'
              }}>
                {tag.toUpperCase()}
              </span>
            ))}
        ...
        {/* styles count badge — top right */}
        <div style={{
          position: 'absolute', top: '24px', right: '32px', zIndex: 3,
          background: 'rgba(255,255,255,0.45)', backdropFilter: 'blur(14px)',
          borderRadius: '12px', padding: '10px 18px', border: '1px solid rgba(255,255,255,0.6)',
          textAlign: 'center',
        }}>
          <span style={{ color: '#2c221e', fontSize: '22px', fontWeight: '700', display: 'block', lineHeight: 1 }}>
            {products.length}
          </span>
          <span style={{ color: '#8c7e75', fontSize: '9px', display: 'block', letterSpacing: '2px', marginTop: '4px', fontWeight: '600' }}>
            STYLES
          </span>
        </div>
      </div>
      ...
      <div style={{
        ...
        padding: '14px 40px',
        display: 'flex', alignItems: 'center', justifyItems: 'center', justifyContent: 'space-between', gap: '16px',
      }}>
      ...
      {activeFilterCount > 0 && (
        <div style={{ padding: '10px 40px', display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center', background: '#f7f7f7' }}>
          {!selectedCats.includes('All') && selectedCats.map(c => (
            <FilterChip key={c} label={c.replace('Girls - ', 'Girls: ').replace('Boys - ', 'Boys: ')} onRemove={() => toggleCat(c)} />
          ))}
          {!selectedAgeGroups.includes('All') && selectedAgeGroups.map(ag => (
            <FilterChip key={ag} label={`Age: ${ag}`} onRemove={() => toggleAgeGroup(ag)} />
          ))}
          {selectedSizes.map(s => (
      ...
      <div style={{ display: 'flex', gap: '0', padding: '0 40px 80px' }}>
        ...
        <div style={{
          ...
          {sidebarOpen && (
            <div>
              {/* Category */}
              <SidebarSection isOpen={openSection.category} onToggle={() => toggleSection('category')} label="CATEGORY">
                {/* Category search */}
                ...
                <div style={{ maxHeight: '320px', overflowY: 'auto', paddingRight: '4px' }}>
                  ...
                  >
                    <span style={{ fontSize: '12px', color: selectedCats.includes('All') ? '#111' : '#999', fontWeight: selectedCats.includes('All') ? '700' : '400' }}>
                      All Kids Clothing
                    </span>
                    {selectedCats.includes('All') && (
                  ...
                  {/* Girls Category list */}
                  <div style={{ marginTop: '12px', borderTop: '1px solid #f9f9f9', paddingTop: '8px' }}>
                    <span style={{ fontSize: '9px', fontWeight: '700', letterSpacing: '1px', color: '#aaa', display: 'block', marginBottom: '6px' }}>GIRLS COLLECTION</span>
                    {visibleGirlsCats.map(c => {
                      const fullKey = `Girls - ${c}`;
                      const active = selectedCats.includes(fullKey);
                      return (
                        <div
                          key={fullKey}
                          onClick={() => toggleCat(fullKey)}
                          style={{
                            display: 'flex', alignItems: 'center', justifyItems: 'center', justifyContent: 'space-between',
                            padding: '6px 0 6px 6px', cursor: 'pointer', borderRadius: '6px',
                          }}
                        >
                          <span style={{ fontSize: '12px', color: active ? '#111' : '#777', fontWeight: active ? '600' : '400' }}>
                            {c}
                          </span>
                          {active && (
                            <div style={{ width: '5px', height: '5px', borderRadius: '50%', background: '#111', flexShrink: 0 }} />
                  ...
                  {/* Boys Category list */}
                  <div style={{ marginTop: '16px', borderTop: '1px solid #f9f9f9', paddingTop: '8px' }}>
                    <span style={{ fontSize: '9px', fontWeight: '700', letterSpacing: '1px', color: '#aaa', display: 'block', marginBottom: '6px' }}>BOYS COLLECTION</span>
                    {visibleBoysCats.map(c => {
                      const fullKey = `Boys - ${c}`;
                      const active = selectedCats.includes(fullKey);
                      return (
                        <div
                          key={fullKey}
              ...
              {/* Age Group */}
              <SidebarSection isOpen={openSection.age} onToggle={() => toggleSection('age')} label="AGE GROUP">
              ...
              {/* Price */}
              <SidebarSection isOpen={openSection.price} onToggle={() => toggleSection('price')} label="PRICE">
                ...
                <input
                  type="range" min="0" max="15000" value={priceRange[1]}
                  onChange={e => setPriceRange([priceRange[0], +e.target.value])}
                  style={{
                    width: '100%', appearance: 'none', height: '3px',
                    background: `linear-gradient(to right, #111 ${(priceRange[1] / 15000) * 100}%, #e0e0e0 ${(priceRange[1] / 15000) * 100}%)`,
                    outline: 'none', border: 'none', cursor: 'pointer', borderRadius: '2px',
                  }}
                />
                {/* Price quick shortcuts */}
                <div style={{ display: 'flex', gap: '6px', marginTop: '14px', flexWrap: 'wrap' }}>
                  {[[0, 3000, 'Under 3K'], [3000, 6000, '3K - 6K'], [6000, 15000, '6K+']].map(([min, max, lbl]) => (
                    <button
                      key={lbl}
              ...
              {/* Sizes */}
              <SidebarSection isOpen={openSection.sizes} onToggle={() => toggleSection('sizes')} label="SIZE">
                  ...
                  {SIZES.map(sz => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          minWidth: '42px', height: '42px', padding: '0 6px',
              ...
              {/* Colors */}
              <SidebarSection isOpen={openSection.colors} onToggle={() => toggleSection('colors')} label="COLOR">
...
/* ─── Sub-components ──────────────────────────────────────────── */
function SidebarSection({ isOpen, onToggle, label, children }) {
  return (
